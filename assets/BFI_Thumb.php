...
if ( ! function_exists( 'bfi_wp_image_editor_check_notice' ) ) {
	function bfi_wp_image_editor_check_notice() {
	    printf( "<div class='error'><p>%s</div>","The server does not have ImageMagick or GD installed and/or enabled! Any of these libraries are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing." );
	}
}
...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_3' ) ) {
	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
	    ...
	    public function negate() {
	        ...
	        return new WP_Error( 'image_negate_error', esc_html__( 'Image negate failed.', 'rentit' ), $this->file );
	    }
	}
}
...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        $basename = rentit_newBasename( $img_path );
	        ...
	        if ( isset( $width ) && $crop_only === false ) {
	            ...
	            $srentit_x = ( isset( $crop_x ) ) ? $crop_x : 0;

	            // get y position to start croping
	            $srentit_y = ( isset( $crop_y ) ) ? $crop_y : 0;

	            // width of the crop
	            if ( isset( $crop_width ) ) {
	                $srentit_w = $crop_width;
	            } else if ( isset( $width ) ) {
	                $srentit_w = $width;
	            } else {
	                $srentit_w = $orig_w;
	            ...
	            $dims = image_resize_dimensions( $srentit_w, $srentit_h, $dst_w, $dst_h, false );
	            $dst_w = $dims[4];
	            $dst_h = $dims[5];

	            // Make the pos x and pos y work with percentages
	            if ( stripos( $srentit_x, '%' ) !== false ) {
	                $srentit_x = (int) ( (float) str_replace( '%', '', $width ) / 100 * $orig_w );
	            }
	            if ( stripos( $srentit_y, '%' ) !== false ) {
	                $srentit_y = (int) ( (float) str_replace( '%', '', $height ) / 100 * $orig_h );
	            }

	            // allow center to position crop start
	            if ( $srentit_x === 'center' ) {
	                $srentit_x = ( $orig_w - $srentit_w ) / 2;
	            }
	            if ( $srentit_y === 'center' ) {
	                $srentit_y = ( $orig_h - $srentit_h ) / 2;
	            }
	        }
	        ...
	        $suffix = (string) filemtime( $img_path ) .
	            ...
	            ( isset( $srentit_x ) ? str_pad( (string) $srentit_x, 5, '0', STR_PAD_LEFT ) : '00000' ) .
	            ( isset( $srentit_y ) ? str_pad( (string) $srentit_y, 5, '0', STR_PAD_LEFT ) : '00000' ) .
	            ( isset( $srentit_w ) ? str_pad( (string) $srentit_w, 5, '0', STR_PAD_LEFT ) : '00000' ) .
	            ( isset( $srentit_h ) ? str_pad( (string) $srentit_h, 5, '0', STR_PAD_LEFT ) : '00000' ) .
	        ...
	        $dst_rel_path = str_replace( '.' . $ext, '', rentit_newBasename( $img_path ) );
	        ...
	        if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
	            ...
	            if ( $crop_only === false ) {
	                ...
	                if ( is_wp_error( $editor->crop( $srentit_x, $srentit_y, $srentit_w, $srentit_h, $dst_w, $dst_h ) ) ) {
	                    return false;
	                }
	            }
...
if ( ! function_exists( 'bfi_image_resize_dimensions' ) ) {
	function bfi_image_resize_dimensions( $payload, $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
	    ...
	    // int dst_x, int dst_y, int srentit_x, int srentit_y, int dst_w, int dst_h, int srentit_w, int srentit_h
...
function rentit_get_thumbnail($theid) {
	$thumbnail = get_the_post_thumbnail($theid, 'rentit_panorama');
	preg_match_all('#src="(.*?)"#si', $thumbnail, $thumb_url);
	$thumb = "";
	if (isset($thumb_url[1][0])) $thumb = esc_url($thumb_url[1][0]);
	return $thumb;
}



function  rentit_get_big_img($post_id){
$big_img = wp_get_attachment_url(get_post_meta($post_id, '_big_img', true));
$params = array('width' => 1000, 'height' => 600);
	return  bfi_thumb($big_img, $params);
